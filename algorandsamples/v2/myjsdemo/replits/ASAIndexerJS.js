...
(async () => {
    ...
    // SEARCH BALANCES
    await assetBalances();   
    await searchAssetName();
    await searchTxAddressAsset();         
    await searchAssets();
...
async function searchAssetName() {
    console.log("");
    ...
    const assetInfo = await indexerClient.searchForAssets()
        .name(name)
        .creator(creator).do();
    console.log("Creator Information for Asset Name: " + JSON.stringify(assetInfo, undefined, 2));
}

async function assetBalances() {
    console.log("");
...
async function searchTxAddressAsset() {
    console.log("");
...
async function searchAssets() {
    ...
    console.log("Account Information for Asset: " + JSON.stringify(accountInfo, undefined, 2));
}
