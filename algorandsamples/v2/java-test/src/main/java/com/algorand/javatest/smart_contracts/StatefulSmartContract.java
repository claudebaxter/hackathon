...
public class StatefulSmartContract {
    ...
    public Long createApp(AlgodClient client, Account creator, TEALProgram approvalProgramSource,
        ...
        Transaction txn = Transaction.ApplicationCreateTransactionBuilder()
                .sender(sender).suggestedParams(params)
                ...
                .localStateSchema(new StateSchema(localInts, localBytes)).build();
    ...
    public void readLocalState(AlgodClient client, Account account, Long appId) throws Exception {
        Response<com.algorand.algosdk.v2.client.model.Account> acctResponse = client
        ...
        for (int i = 0; i < applicationLocalState.size(); i++) {
            if (applicationLocalState.get(i).id.equals(appId)) {
                System.out
                        .println("User's application local state: " + applicationLocalState.get(i).keyValue.toString());
            }
        }
    }
    ...
    public void readGlobalState(AlgodClient client, Account account, Long appId) throws Exception {
        ...
        for (int i = 0; i < createdApplications.size(); i++) {
            if (createdApplications.get(i).id.equals(appId)) {
                System.out.println(
                        "Application global state: " + createdApplications.get(i).params.globalState.toString());
            }
        }
    }

    public void updateApp(AlgodClient client, Account creator, Long appId, TEALProgram approvalProgramSource,
            TEALProgram clearProgramSource) throws Exception {
        // define sender as creator
        Address sender = creator.getAddress();
        ...
        // create unsigned transaction
        Transaction txn = Transaction.ApplicationUpdateTransactionBuilder().sender(sender).suggestedParams(params)
                .applicationId(appId).approvalProgram(approvalProgramSource).clearStateProgram(clearProgramSource)
                .build();
    ...
    public void closeOutApp(AlgodClient client, Account userAccount, Long appId) throws Exception {
        ...
        // create unsigned transaction
        Transaction txn = Transaction.ApplicationCloseTransactionBuilder().sender(sender).suggestedParams(params)
                .applicationId(appId).build();
    ...
    public void clearApp(AlgodClient client, Account userAccount, Long appId) throws Exception {
        ...
        // create unsigned transaction
        Transaction txn = Transaction.ApplicationClearTransactionBuilder().sender(sender).suggestedParams(params)
                .applicationId(appId).build();
    ...
    public void deleteApp(AlgodClient client, Account creatorAccount, Long appId) throws Exception {
        ...
        // create unsigned transaction
        Transaction txn = Transaction.ApplicationDeleteTransactionBuilder().sender(sender).suggestedParams(params)
                .applicationId(appId).build();
    ...
    public void statefulSmartContract() throws Exception {
        ...
        // TODO: REMOVE:
        // ABU6EJ637L6BF3MHLPL5FADI7JSM36J3CZILNP256SQ77HCLE2OACD2C3Y
        creatorMnemonic = "tattoo flavor search carry uncover arrange stereo loop crucial bitter captain illness grape deer cup senior patrol online indicate claw puzzle emerge fragile about lemon";
        ...
        String approvalProgramSourceInitial = "#pragma version 5\n"
                ...
                "handle_noop:\n" + "//// Handle NoOp\n" + "//// Check for creator\n"
                + "addr ABU6EJ637L6BF3MHLPL5FADI7JSM36J3CZILNP256SQ77HCLE2OACD2C3Y\n" + "txn Sender\n" + "==\n"
                + "bnz handle_optin\n" +
        ...
        String approvalProgramSourceRefactored = "#pragma version 5\n"
                + "//// Handle each possible OnCompletion type. We don't have to worry about\n"
                + "//// handling ClearState, because the ClearStateProgram will execute in that\n"
                + "//// case, not the ApprovalProgram.\n" +
                ...
                + "addr ABU6EJ637L6BF3MHLPL5FADI7JSM36J3CZILNP256SQ77HCLE2OACD2C3Y\n" + "txn Sender\n" + "==\n"
                + "return\n";
        // creator ABU6EJ637L6BF3MHLPL5FADI7JSM36J3CZILNP256SQ77HCLE2OACD2C3Y
        ...
        try {
            ...
            Long appId = createApp(client, creatorAccount, new TEALProgram(approvalProgram),
                    new TEALProgram(clearProgram), globalInts, globalBytes, localInts, localBytes);
